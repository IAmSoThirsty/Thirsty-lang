...
def transpile_file(file_path: str, target: str = "thirsty") -> None:
    ...
    if target == "thirsty":
        ...
        print(
            f"Error: Unknown target '{target}'. Use 'thirsty' or 'js'.", file=sys.stderr
        )
        sys.exit(1)
...
def main():
    ...
    check_parser = subparsers.add_parser(
        "check", help="Validate deity contract compliance"
    )
    check_parser.add_argument("file", help="Path to .thirstofgods file")

    # transpile
    transpile_parser = subparsers.add_parser(
        "transpile", help="Transpile a .thirstofgods file"
    )
    transpile_parser.add_argument("file", help="Path to .thirstofgods file")
    transpile_parser.add_argument(
        "--target", choices=["thirsty", "js"], default="thirsty", help="Target language"
    )
