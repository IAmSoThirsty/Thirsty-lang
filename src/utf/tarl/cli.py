...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "T.A.R.L. (Thirsty's Active Resistance Language)" " — Policy Evaluator"
        )
    )
    ...
    eval_parser.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    eval_parser.add_argument(
        "--now",
        default=None,
        metavar="ISO8601",
    ...
    lint_parser.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    ...
    audit_chain.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    ...
    audit_query = audit_sub.add_parser("query", help="Query stored evaluation proofs")
    ...
    keygen_parser.add_argument(
        "--key-id",
        required=True,
        help="Identifier stamped into the key",
    )
    keygen_parser.add_argument(
        "--out",
        "-o",
    ...
    revoke_parser.add_argument(
        "--reason",
        default="",
        help="Optional reason recorded with the entry",
    )
    revoke_parser.add_argument(
        "--remove",
        action="store_true",
        help="Un-revoke the given policy hash",
    )
    revoke_parser.add_argument(
        "--list",
        action="store_true",
        help="List all revoked policy hashes",
    )
    revoke_parser.add_argument(
        "--json", "-j", action="store_true", help="Output as JSON"
    )
    ...
    # test
    test_parser = subparsers.add_parser("test", help="Run .tarl_test test suites")
    ...
    analyze_parser.add_argument("policy_files", nargs="+", help="Policy file(s)")
    analyze_parser.add_argument(
        "--json", "-j", action="store_true", help="Output as JSON"
    )
...
def _cmd_verify(args):
    ...
    for key_path in getattr(args, "ed25519_key_file", None) or []:
        ...
        try:
            ...
            print(f"Invalid --ed25519-key-file {key_path!r}: {e}", file=sys.stderr)
            sys.exit(1)
    ...
    if raw_evaluated_context is not None:
        try:
            expected_evaluated_context = load_context_json(raw_evaluated_context)
        except (json.JSONDecodeError, ContextResolutionError) as e:
            print(
    ...
    if raw_now is not None:
        try:
            ...
            sys.exit(1)
        if verification_now.tzinfo is None or verification_now.utcoffset() is None:
            print(
...
def _cmd_audit(args):
    ...
    if args.audit_command != "query":
        print(
            "Usage: tarl audit (query | verify-chain | checkpoint) " "[options]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _cmd_revoke(args):
    ...
    with RevocationStore(args.store) as store:
        ...
        if not args.policy_hash:
            print(
                "Usage: tarl revoke <policy_hash> | --list | " "--remove <policy_hash>",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if args.remove:
            ...
            print(f"{'Removed' if removed else 'Not present'}: " f"{args.policy_hash}")
            sys.exit(0)

        added = store.add(args.policy_hash, reason=args.reason)
        print(f"{'Revoked' if added else 'Already revoked'}: " f"{args.policy_hash}")
        sys.exit(0)
