...
class TestSafeExpr:
    ...
    def test_and_operator(self):
        assert (
            SafeExpr.evaluate(
                'role == "admin" and level >= 3', {"role": "admin", "level": 5}
            )
            is True
        )
        assert (
            SafeExpr.evaluate(
                'role == "admin" and level >= 3', {"role": "user", "level": 5}
            )
            is False
        )
        assert (
            SafeExpr.evaluate(
                'role == "admin" and level >= 3', {"role": "admin", "level": 1}
            )
            is False
        )
    ...
    def test_or_operator(self):
        assert (
            SafeExpr.evaluate(
                'role == "admin" or level >= 3', {"role": "admin", "level": 1}
            )
            is True
        )
        assert (
            SafeExpr.evaluate(
                'role == "admin" or level >= 3', {"role": "user", "level": 5}
            )
            is True
        )
        assert (
            SafeExpr.evaluate(
                'role == "admin" or level >= 3', {"role": "user", "level": 1}
            )
            is False
        )
...
class TestTarlRuntime:
    ...
    def test_first_match_wins_survives_adaptive_ordering(self):
        ...
        assert (
            result.verdict == TarlVerdict.ALLOW
...
class TestThrowStats:
    ...
    def test_dead_by_exception_predicate(self):
        ...
        policy_text = f"{self._THROW_RULE}\nwhen true => ALLOW"
        for _ in range(5):
            rt.evaluate({"risk_score": "not-a-number"}, policy_text=policy_text)
        stats = rt.throw_stats()
        assert (
            stats.get(0, 0) > 0
        ), "throw_count should be nonzero for always-throwing rule"
        assert (
            rt._hit_counts.get(0, 0) == 0
        ), "hit_count must be zero for a never-matching rule"
