...
def _policy(rules_text: str, name: str = "test") -> TarlPolicy:
    text = f"policy {name}:\n" + "\n".join(f"  when {c} => {v}" for c, v in rules_text)
...
class TestAnalysisResult(unittest.TestCase):
    ...
    def test_str_unavailable(self):
        r = AnalysisResult(
            kind="coverage", available=False, passed=False, message="no z3"
        )
        self.assertIn("unavailable", str(r))
        self.assertIn("coverage", str(r))

    def test_str_pass(self):
        r = AnalysisResult(kind="shadows", available=True, passed=True, message="clean")
        self.assertIn("PASS", str(r))
        self.assertIn("shadows", str(r))

    def test_str_fail(self):
        r = AnalysisResult(
            kind="conflicts", available=True, passed=False, message="found 1"
        )
    ...
    def test_summary_with_shadows(self):
        r = AnalysisResult(
            ...
            message="dead rule",
            shadows=[
                ShadowedRule(rule_index=1, condition="x > 0", description="shadowed")
            ],
        )
...
class TestAnalyzerZ3Unavailable(unittest.TestCase):
    ...
    def test_coverage_unavailable(self):
        r = self._run_with_no_z3(lambda: PolicyAnalyzer(self.policy).check_coverage())
        self.assertFalse(r.available)
        self.assertEqual(r.kind, "coverage")

    def test_shadows_unavailable(self):
        r = self._run_with_no_z3(lambda: PolicyAnalyzer(self.policy).check_shadows())
        self.assertFalse(r.available)
        self.assertEqual(r.kind, "shadows")

    def test_conflicts_unavailable(self):
        r = self._run_with_no_z3(lambda: PolicyAnalyzer(self.policy).check_conflicts())
        self.assertFalse(r.available)
...
class TestConditionToZ3TypeInference(unittest.TestCase):
    ...
    def _infer(self, *conditions) -> dict:
        tr = _ConditionToZ3()
        rules = [TarlRule(condition=c, verdict=TarlVerdict.ALLOW) for c in conditions]
...
class TestRefinesAnalysis(unittest.TestCase):
    ...
    def test_strict_subset_passes(self):
        ...
        permissive = PolicyParser.parse("policy perm:\n  when x > 0 => ALLOW")
...
class TestTemporalBuiltinsInZ3(unittest.TestCase):
    ...
    def test_current_hour_has_domain(self):
        ...
        toks = PolicyParser._tokenize("CURRENT_HOUR >= 9 and CURRENT_HOUR < 17")
        ast = SafeExpr(toks).parse_expr()
        formula = tr.to_bool(ast)
        s = z3.Solver()
        s.add(*tr.domain_constraints())
    ...
    def test_current_hour_domain_respected(self):
        ...
        tr = _ConditionToZ3()
        rules = [TarlRule(condition="CURRENT_HOUR >= 0", verdict=TarlVerdict.ALLOW)]
        tr.infer_types(rules)
